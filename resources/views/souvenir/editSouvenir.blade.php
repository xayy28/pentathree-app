...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl sm:text-3xl font-serif font-semibold text-[#2C3E35] mb-1">
                Edit Souvenir: {{ $souvenir->nama_souvenir }}
            </h1>
            <p class="text-xs sm:text-sm text-[#5C6E65]">
                Ubah informasi souvenir melalui formulir di bawah ini.
            </p>
        </div>
        <a href="{{ route('admin.souvenir') }}" class="px-4 py-2 border border-[#E6E4DD] hover:bg-[#FAF9F6] text-[#2C3E35] text-xs font-semibold rounded-lg transition-colors flex items-center gap-1.5">
    ...
    <div class="bg-white rounded-2xl border border-[#E6E4DD] p-6 sm:p-8 shadow-sm">
        <form action="{{ route('admin.souvenir.update', $souvenir->souvenir_id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- Nama Souvenir -->
                <div class="space-y-2">
                    <label for="nama_souvenir" class="text-xs font-bold uppercase tracking-wider text-[#8A9C91]">Nama Souvenir</label>
                    <input type="text" 
                           id="nama_souvenir" 
                           name="nama_souvenir" 
                           value="{{ old('nama_souvenir', $souvenir->nama_souvenir) }}" 
                           placeholder="Masukkan nama souvenir"
                           class="w-full px-4 py-3 bg-[#FAF9F6] border border-[#E6E4DD] focus:border-[#2B4C3F] focus:ring-1 focus:ring-[#2B4C3F] rounded-xl text-[#2C3E35] placeholder-[#8A9C91] text-sm outline-none transition-all @error('nama_souvenir') border-red-500 @enderror">
                    @error('nama_souvenir')
                ...
                <!-- Harga -->
                <div class="space-y-2">
                    <label for="harga" class="text-xs font-bold uppercase tracking-wider text-[#8A9C91]">Harga (Rp)</label>
                    <div style="position: relative; display: flex; align-items: center;">
                        ...
                        <input type="number" 
                               id="harga" 
                               name="harga" 
                               value="{{ old('harga', (int)$souvenir->harga) }}" 
                               placeholder="15000"
                               style="padding-left: 2.75rem;"
                               class="w-full pr-4 py-3 bg-[#FAF9F6] border border-[#E6E4DD] focus:border-[#2B4C3F] focus:ring-1 focus:ring-[#2B4C3F] rounded-xl text-[#2C3E35] placeholder-[#8A9C91] text-sm outline-none transition-all @error('harga') border-red-500 @enderror">
                    </div>
                    @error('harga')
                        <p class="text-xs text-[#E65F5F] font-semibold mt-1">{{ $message }}</p>
                    @enderror
                ...
                <!-- Stok -->
                <div class="space-y-2">
                    <label for="stok" class="text-xs font-bold uppercase tracking-wider text-[#8A9C91]">Stok (pcs)</label>
                    <input type="number" 
                           id="stok" 
                           name="stok" 
                           value="{{ old('stok', $souvenir->stok) }}" 
                           placeholder="Jumlah stok barang"
                           class="w-full px-4 py-3 bg-[#FAF9F6] border border-[#E6E4DD] focus:border-[#2B4C3F] focus:ring-1 focus:ring-[#2B4C3F] rounded-xl text-[#2C3E35] placeholder-[#8A9C91] text-sm outline-none transition-all @error('stok') border-red-500 @enderror">
                    @error('stok')
                        <p class="text-xs text-[#E65F5F] font-semibold mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <select id="status" 
                        ...
                        <option value="Tersedia" {{ old('status', $souvenir->status) == 'Tersedia' ? 'selected' : '' }}>Tersedia</option>
                        <option value="Habis" {{ old('status', $souvenir->status) == 'Habis' ? 'selected' : '' }}>Habis</option>
                    </select>
                    @error('status')
                        <p class="text-xs text-[#E65F5F] font-semibold mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div class="space-y-2">
                ...
                <textarea id="detail" 
                          name="detail" 
                          rows="4" 
                          placeholder="Jelaskan detail, ukuran, bahan, dan deskripsi souvenir di sini..."
                          class="w-full px-4 py-3 bg-[#FAF9F6] border border-[#E6E4DD] focus:border-[#2B4C3F] focus:ring-1 focus:ring-[#2B4C3F] rounded-xl text-[#2C3E35] placeholder-[#8A9C91] text-sm outline-none transition-all @error('detail') border-red-500 @enderror">{{ old('detail', $souvenir->detail) }}</textarea>
                @error('detail')
                    <p class="text-xs text-[#E65F5F] font-semibold mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <!-- Upload Foto -->
            <div class="space-y-2">
                <label class="text-xs font-bold uppercase tracking-wider text-[#8A9C91] block">Foto Souvenir</label>
                ...
                @if($souvenir->foto)
                    <div id="existing-image-container" style="margin-bottom: 12px;">
                        ...
                        <img src="{{ asset($souvenir->foto) }}" alt="{{ $souvenir->nama_souvenir }}" style="width: 100%; max-width: 320px; height: 180px; object-fit: cover; border-radius: 14px; border: 1.5px solid #E6E4DD; box-shadow: 0 2px 12px rgba(0,0,0,0.06);">
                    </div>
                @endif
                ...
                <div id="image-preview-container" style="display: none; margin-top: 12px;">
                    <p style="font-size: 0.7rem; color: #8A9C91; margin-bottom: 8px;">Pratinjau Foto Baru:</p>
                    <img id="image-preview" src="#" alt="Pratinjau Baru" style="width: 100%; max-width: 320px; height: 180px; object-fit: cover; border-radius: 14px; border: 1.5px solid #E6E4DD; box-shadow: 0 2px 12px rgba(0,0,0,0.06);">
                </div>
                @error('foto')
                    <p class="text-xs text-[#E65F5F] font-semibold mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="pt-4 border-t border-[#F2F0EA] flex justify-end gap-3">
                <a href="{{ route('admin.souvenir') }}" class="px-5 py-2.5 bg-[#FAF9F6] border border-[#D5D3C7] hover:bg-[#F2F0EA] text-[#2C3E35] text-sm font-semibold rounded-xl transition-all">
                    Batalkan
                </a>
