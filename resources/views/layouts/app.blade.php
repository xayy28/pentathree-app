...
<body class="bg-[#F8F7F4] text-[#2C3E35] font-sans min-h-screen flex">
    ...
    <aside class="w-64 bg-[#2B4C3F] text-[#FAF9F6] hidden md:flex flex-col flex-shrink-0 shadow-lg">
        ...
        <div class="flex-grow flex flex-col justify-between py-6">
            ...
            <nav class="px-4 space-y-1.5">
                @if(auth()->user()->role === 'admin')
                    ...
                    <a href="{{ route('admin.fasilitas') }}"
                        class="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('admin.fasilitas*') ? 'bg-[#3B5D50] text-white' : 'text-[#A7C5B5] hover:bg-[#325547] hover:text-white' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                            xmlns="http://www.w3.org/2000/svg">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="px-4">
                <x-ui-button type="button" onclick="event.preventDefault(); document.getElementById('logout-form').submit();" variant="danger" block>
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                        xmlns="http://www.w3.org/2000/svg">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
