    ...
    <div class="invoice-shell max-w-4xl mx-auto {{ $isAdmin ? '' : 'px-4 sm:px-6 lg:px-8 py-10' }}">
        <div class="no-print mb-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <div class="flex items-center gap-3">
                @if ($isAdmin)
                    <x-ui-button href="{{ route('admin.invoices.pdf', $invoice->invoice_id) }}" variant="primary">
                ...
                @else
                    <x-ui-button href="{{ route('user.invoices.pdf', $invoice->pemesanan_id) }}" variant="primary">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                        </svg>
                        Unduh PDF
