            ...
            <div class="space-y-2">
                ...
                @if($homestay->foto)
                    <div id="existing-image-container" class="mb-4">
                        <p class="text-xs text-[#8A9C91] mb-2">Foto Saat Ini:</p>
                        <img src="{{ asset($homestay->foto) }}" alt="{{ $homestay->nama_homestay }}" class="w-full max-w-md h-64 object-cover rounded-2xl border border-[#E6E4DD]">
                    </div>
                @endif
                ...
                <div id="image-preview-container" class="hidden mt-4">
                    <p class="text-xs text-[#8A9C91] mb-2">Pratinjau Foto Baru:</p>
                    <img id="image-preview" src="#" alt="Pratinjau Baru" class="w-full max-w-md h-64 object-cover rounded-2xl border border-[#E6E4DD]">
                </div>
                @error('foto')
                    <p class="text-xs text-[#E65F5F] font-semibold mt-1">{{ $message }}</p>
                @enderror
    ...
    function previewImage(event) {
        ...
        if (input.files && input.files[0]) {
            ...
            reader.onload = function(e) {
                ...
                if (existing) {
                    existing.classList.add('opacity-50');
                }
            }
            ...
            if (existing) {
                existing.classList.remove('opacity-50');
            }
        }
    }
</script>
