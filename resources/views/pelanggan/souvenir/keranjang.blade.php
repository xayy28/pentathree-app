    ...
    @section('content')
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-12 bg-[#F8F7F4]">
            ...
            @if($items->isEmpty())
                <!-- Empty State -->
                <div
                    class="max-w-md mx-auto bg-white rounded-3xl border border-[#E6E4DD] p-12 text-center flex flex-col items-center justify-center shadow-sm">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-10 items-start">
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        @foreach($items as $item)
                            ...
                            @endphp
                            <div
                                class="bg-white rounded-3xl p-6 border border-[#E6E4DD]/60 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-6 transition-all duration-300 hover:shadow-md">

                                <!-- Product Info (Image + Title + Price) -->
                                <div class="flex items-center gap-5 w-full sm:w-auto">
                                    <div
                                        class="w-20 h-20 rounded-2xl overflow-hidden bg-[#EAF2EE]/50 flex-shrink-0 border border-[#E6E4DD]/40">
                                        @if($souvenir->foto)
                                    ...
                                    <div class="space-y-1">
                                        ...
                                        <div class="text-xs text-[#8A9C91]">
                                            Harga: <span class="font-semibold text-[#2B4C3F]">Rp
                                ...
                                <div
                                    class="flex items-center justify-between sm:justify-end gap-8 w-full sm:w-auto border-t sm:border-t-0 pt-4 sm:pt-0 border-[#F2F0EA]">
                                    ...
                                    <div class="flex items-center border border-[#E6E4DD] rounded-xl overflow-hidden bg-[#FAF9F6]">
                                        ...
                                        <form action="{{ route('cart.update') }}" method="POST" class="inline">
                                            ...
                                            <button type="submit"
                                                class="px-3 py-2 text-gray-500 hover:bg-[#EAF2EE] hover:text-[#2B4C3F] transition-all disabled:opacity-30 disabled:cursor-not-allowed"
                                        ...
                                        <!-- Quantity Display -->
                                        <span
                                            class="px-4 py-1 text-sm font-semibold text-[#2C3E35] select-none min-w-[32px] text-center">
                                            {{ $item->quantity }}
                                        </span>
                                        ...
                                        <form action="{{ route('cart.update') }}" method="POST" class="inline">
                                            ...
                                            <button type="submit"
                                                class="px-3 py-2 text-gray-500 hover:bg-[#EAF2EE] hover:text-[#2B4C3F] transition-all disabled:opacity-30 disabled:cursor-not-allowed"
                                                {{ $item->quantity >= $souvenir->stok ? 'disabled' : '' }}>
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                                                    xmlns="http://www.w3.org/2000/svg">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const serviceFee = {{ $items->isEmpty() ? 0 : 5000 }};
        ...
        function updateSummary() {
            ...
            if (summaryCount) summaryCount.textContent = totalQuantity;
            if (summarySubtotal) summarySubtotal.textContent = formatRupiah(subtotal);
            
            // Jika ada barang yang diceklis, tambahkan service fee
            const finalTotal = subtotal > 0 ? subtotal + serviceFee : 0;
            if (summaryTotal) summaryTotal.textContent = formatRupiah(finalTotal);
            
            // Disable tombol checkout jika tidak ada item yang dipilih
            if (btnCheckout) {
                if (totalQuantity === 0) {
        ...
        // Listener untuk tombol "Pilih Semua"
        const selectAllCb = document.getElementById('select-all');
        if (selectAllCb) {
        ...
        if (btnCheckout) {
            btnCheckout.addEventListener('click', function(e) {
                ...
                checkboxes.forEach(cb => {
                    if (cb.checked) {
                        selectedParams.push('selected_items[]=' + encodeURIComponent(cb.value));
                    }
                });
                
                if (selectedParams.length > 0) {
                    const url = "{{ route('checkout.index') }}?" + selectedParams.join('&');
