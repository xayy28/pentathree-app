            ...
            @else
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8" id="souvenir-grid">
                    @foreach($souvenirs as $souvenir)
                        ...
                        @endphp
                        <div class="bg-white rounded-[32px] overflow-hidden border border-gray-200/60 shadow-sm hover:shadow-md transition-all duration-300 transform hover:-translate-y-0.5 group flex flex-col justify-between p-4 souvenir-card" data-status="{{ strtolower($souvenir->status) }}">
                            <div>
                                <!-- Image Container -->
                                <div class="h-60 overflow-hidden relative bg-[#EAF2EE]/50 rounded-2xl">
                                    @if($souvenir->foto)
                                        <img src="{{ asset($souvenir->foto) }}" alt="{{ $souvenir->nama_souvenir }}" class="w-full h-full object-cover rounded-2xl group-hover:scale-[1.02] transition-transform duration-500">
                                    @else
                                        <div class="w-full h-full flex items-center justify-center text-6xl bg-[#FAF9F6] rounded-2xl">🏺</div>
                                    @endif
                            ...
                            <div class="p-4 pt-0">
                                <div class="flex items-center justify-between border-t border-gray-100 pt-4">
                                    <div>
                                        ...
                                        <span class="text-base font-bold text-[#1E362C]">Rp {{ number_format($souvenir->harga, 0, ',', '.') }}</span>
                                    </div>
                                    <div class="text-right">
                                        <span class="text-[10px] text-[#8A9C91] block font-medium">Stok: {{ $souvenir->stok }} pcs</span>
                                    </div>
                                </div>
                            </div>
                        </div>
